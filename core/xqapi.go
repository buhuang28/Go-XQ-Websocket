...
// remark  附加信息  文本型  拒绝入群，拒绝添加好友 附加信息
func HandleGroupEvent(selfID int64, sub_type int64, userID int64, groupID int64, flag string, approve int64, remark string) {
	C.S3_Api_HandleGroupEvent(
		GoInt2CStr(selfID), C.int(sub_type), GoInt2CStr(userID), GoInt2CStr(groupID), CString(flag), C.int(approve), CString(remark),
	)
}
